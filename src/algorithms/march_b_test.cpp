...
const char* MarchB::name() const
{
    return "March B";
}

int MarchB::run(FaultyMemory *mem)
{
    ...
    emit message("M0: Запись всех ячеек в 0 (восходящий)...");
    ...
    // M2: ↑(r1,w0) - Чтение 1, запись 0 (восходящий)
    emit message("M2: Чтение 1 и запись 0 (восходящий)...");
    for (size_t i = 0; i < n; i++)
    {
        // r1: Чтение 1
        uint8_t val = mem->read(i);
    ...
    // M3: ↓(r0,w1) - Чтение 0, запись 1 (нисходящий)
    emit message("M3: Чтение 0 и запись 1 (нисходящий)...");
    for (size_t i = n; i-- > 0;)
    {
        // r0: Чтение 0
        uint8_t val = mem->read(i);
    ...
    // M4: ↓(r1,w0) - Чтение 1, запись 0 (нисходящий)
    emit message("M4: Чтение 1 и запись 0 (нисходящий)...");
    for (size_t i = n; i-- > 0;)
    {
        // r1: Чтение 1
        uint8_t val = mem->read(i);
